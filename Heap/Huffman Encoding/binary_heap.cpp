...
binary_heap::binary_heap(vector<*huffman_node> vec) : heap_size(vec.size()) {
    heap = vec;
...
void binary_heap::insert(*huffman_node x) {
...
void binary_heap::percolateUp(*huffman_node hole) {
    // get the value just inserted
    *huffman_node x = heap[hole];
...
void binary_heap::percolateDown(*huffman_node hole) {
    // get the value to percolate down
    *huffman_node x = heap[hole];
    // while there is a left child...
    while ( hole*2 <= heap_size ) {
        *huffman_node child = hole*2; // the left child
...
*huffman_node binary_heap::findMin() {
    if ( heap_size == 0 )
